...
export function AdminPage() {
  ...
  const [tab, setTab] = useState<'users' | 'sessions' | 'products'>('users');
  ...
  const handleSaveProduct = async () => {
    ...
    setShowForm(false);
    setEditingId(null);
    setForm({ title: '', description: '', price: 0, image: '', category: '' });
    await loadData();
  };
  ...
  return (
    <div>
      ...
      <div className="flex gap-4 mb-8 border-b border-gray-200">
        {(['users', 'sessions', 'products'] as const).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
          ...
          {sessions.map((s) => (
            <div key={s.id} className="bg-white rounded-lg border border-gray-200 p-4">
              <div className="flex justify-between">
                <span className="font-medium">{s.getDeviceLabel()} — {s.deviceType}</span>
          ...
          {showForm && (
            <div className="bg-white rounded-xl border border-gray-200 p-6 mb-6 space-y-4">
              ...
              <div className="flex gap-2">
                <button onClick={handleSaveProduct} className="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm">
                  Save
                </button>
                ...
                {products.map((p) => (
                  <tr key={p.getId()}>
                    ...
                    <td className="px-4 py-3 space-x-2">
                      ...
                      <button onClick={() => handleDelete(p.getId())} className="text-red-600 text-sm">Delete</button>
                    </td>
                  </tr>
                ))}
              </tbody>
